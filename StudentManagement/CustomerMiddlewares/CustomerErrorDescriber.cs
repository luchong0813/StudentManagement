...
namespace StudentManagement.CustomerMiddlewares
{
    public class CustomerErrorDescriber : IdentityErrorDescriber
    {
        ...
        {
            return new IdentityError { Code = nameof(LoginAlreadyAssociated), Description = "该用户名已存在" };
        }

        public override IdentityError InvalidUserName(string userName)
        {
            return new IdentityError { Code = nameof(InvalidUserName), Description = $"用户名{userName}无效，只能包含字母和数字" };
        }
        ...
        {
            return new IdentityError { Code = nameof(PasswordTooShort), Description = $"密码必须至少是{length}个字符." };
        }
